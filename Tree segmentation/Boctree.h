...
class bitoct{
  ...
  static inline void link(bitoct &parent, bitunion<T> *child) {
    parent.child_pointer = (long long)((char*)child - (char*)&parent);      //可用void*来代替
  }
...
template <class T> union bitunion {
  ...
  static inline void link(bitunion<T>* leaf, pointrep* points) {
    ...
    leaf->node.child_pointer = (long long)((char*)points - (char*)leaf);
  }
...
//            p[0] = point[0]; p[1] = point[1]; p[2] = point[2];
  ...
  void GetOctTreeRandom(vector<T*>&c, bitoct &node) {
    bitunion<T> *children;
	//PackedChunkAllocator* tt = (PackedChunkAllocator*)alloc;
	//tt->printSize(o);
	//o<<"node valid "<<node.valid<<" node leaf "<<node.leaf<<","<<(long long)((char*)&node)<<","<<node.child_pointer<<","<<"\n"<<std::flush;
	//o<<"(char*)&node + node.child_pointer"<<(char*)&node + (long)node.child_pointer<<"\n"<<std::flush;
	//long long temp = (long long)((char*)&node)+node.child_pointer;
	//o<<"temp:"<<temp<<"\n"<<std::flush;
	
	//判断是否越界
	//if((temp+node.child_pointer) > 2147483647  || (temp+node.child_pointer)< (-2147483648))
	//{
	//	o<<"long long yuejie"<<temp<<","<<node.child_pointer<<"\n"<<std::flush;
	//	return;
	//}
	children = (bitunion<T>*)((char*)&node + node.child_pointer);
	//o<<"children valid "<<children->node.valid<<" children leaf "<<children->node.leaf<<","<<children->node.child_pointer<<"\n"<<std::flush;
	//if(node.valid<0 || node.valid>255 || node.leaf<0 || node.leaf>255||node.child_pointer<0)
	//	return;
	//unsigned int 0~4294967295
	//int -2147483648~2147483647
	//unsigned long 0~4294967295
	//long -2147483648~2147483647
	//long long -9223372036854775808~9223372036854775807 __int64
	...
	for (short i = 0; i < 8; i++) {
		...
		if (  ( 1 << i ) & node.leaf ) {   // if ith node is leaf
			...
			*/  
			...
			{
				//o<<"Oh Now! Unknown exception"<<"\n"<<std::flush;
			}

		} else { // recurse
			//o<<"Else children node valid: "<<children->node.valid<<" children leaf "<<children->node.leaf<<","<<children->node.child_pointer<<"\n"<<std::flush;
			if(children->node.valid > 0 )
			{
				//o<<"node valid leaf invalid "<<"\n"<<std::flush;
				try{
...
/*  template <class P>
  void countPointsAndQueueFast(P * const* points, int n,  T center[8][3], T size, bitoct &parent, T pcenter[3]) {
    ...
    bitunion<T> *children = alloc->allocate<bitunion<T> >(n_children);
    bitoct::link(parent, children);
	//long long test = (long long)((char*)children);
	//long long test2 = (long long)((char*)&parent);
	//o<<"parent child pointer:"<<n_children<<","<<test<<","<<test2<<","<<(long long)((char*)children - (char*)&parent)<<","<<parent.child_pointer<<"\n"<<std::flush;
    int count = 0;
    for (int j = 0; j < 8; j++) {
      if (blocks[j+1] - blocks[j] > 0) {
        pointrep *c = (pointrep*)branch(children[count].node, blocks[j], blocks[j+1] - blocks[j], center[j], size,o);  // leaf node
        if (c) {
          // absolute pointer
