...
CircularBuffer::CircularBuffer(double radius, double scaling_factor)
{
	radius_ = radius;
	scaling_factor_ = scaling_factor;
	
	unsigned int scaled_radius = unsigned int(scaling_factor*radius); 
	...
	for (unsigned int j = 0; j < kernel_size; j++){
		...
		for (unsigned int k = 0; k < kernel_size; k++){
			...
			if ((xx*xx + yy*yy) <= squared_radius){
				...
				std::array<int,2>temparray = {xx,yy};
				coordinate_offsets_.push_back(temparray); 
