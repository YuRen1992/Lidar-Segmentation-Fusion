...
namespace NWUClustering
{
	...
	void ClusteringAlgo::writeClusters_uf(std::vector<MYPointCTreeID>& treeseed)
	{
		// writing point id and cluster id pairs per line, noise has cluster id 0	
		vector <int> clusters;
		...
		for(i = 0; i < m_pts->m_i_num_points; i++)
		{
			...
			//added by sunxl at 2016/05/16
			if (root < 0 || root >= m_parents.size())
			{
				continue;
			}
		...
		//	//for (j = 0; j < m_pts->m_i_dims; j++)
  //          		//	o << " " << m_pts->m_points[i][j];	
		//	o <<m_pts->m_points[i][0]<<","<<m_pts->m_points[i][1]<<","<<m_pts->m_points[i][2]<<","<< clusters[m_parents[i]]<<endl;
		//	//o << i << " " << clusters[m_parents[i]] << endl;
		//}
		Points* m_centerpts = new Points;
		m_centerpts->m_points.resize(count);
		for(int ll = 0; ll < count; ll++)
			m_centerpts->m_points[ll].resize(4);
		for(i = 0;i<m_pts->m_i_num_points; i++)
		{
			int id = clusters[m_parents[i]];
			if(id == 0)
				continue;
			m_centerpts->m_points[id-1][0] += m_pts->m_points[i][0];
			m_centerpts->m_points[id-1][1] += m_pts->m_points[i][1];
			m_centerpts->m_points[id-1][2] += m_pts->m_points[i][2];
			m_centerpts->m_points[id-1][3] +=1;;
		}
		//header
		//o<< "TreeLocationX,TreeLocationY,TreeHeight,CrownDiameter,CrownArea,CrownVolume\n"<<std::flush;
		for(i =0;i<count;i++)
		{
			m_centerpts->m_points[i][0] = m_centerpts->m_points[i][0]/m_centerpts->m_points[i][3];
			m_centerpts->m_points[i][1] = m_centerpts->m_points[i][1]/m_centerpts->m_points[i][3];
			m_centerpts->m_points[i][2] = m_centerpts->m_points[i][1]/m_centerpts->m_points[i][3];
			MYPointCTreeID temppoint;
			temppoint.x = m_centerpts->m_points[i][0];
			temppoint.y = m_centerpts->m_points[i][1];
			temppoint.z = m_centerpts->m_points[i][2];
			//temppoint.treeID = i+1;
			treeseed.push_back(temppoint);
			//o <<m_centerpts->m_points[i][0]<<","<<m_centerpts->m_points[i][1]<<","<<m_centerpts->m_points[i][2]<<","<<0<<","<<0<<","<<0<<endl;
		}
		cout << "Total points " << noise + sum_points << " pt_in_cls " << sum_points << " noise " << noise << endl;
		cout << "Number of clusters: " << count << endl;
        m_centerpts->m_points.clear();
		delete m_centerpts;
		m_centerpts = NULL;
		clusters.clear();
	}
