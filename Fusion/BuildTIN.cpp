...
static double orient2d(const float *pa, const double* dpb, const float* pc)
{
 ...
 bool BuildTIN::TINinit(float* v0, float* v1, float* v2)
{
	int i, j, k;
  ...
  int BuildTIN::TINinsegment(const float* a, const float* b, const float* p)
{
	if (a[0] < b[0])
	{
		if ((p[0] < a[0]) || (p[0] > b[0]))
 ...
 bool BuildTIN::TINonsegment(const float* a, const float* b, const float* p)
{
	if (a[0] < b[0])
	{
 ...
 bool BuildTIN::TINincircle(const TINtriangle* t, const float* p)
{
 ...
 TINtriangle* BuildTIN::TINlocate_brute(float* p)
{
	int i;
	TINtriangle* t;
	double d[3];
...
// special handling if p is *on* the line defined by the finite edge of the infinite triangle
 TINtriangle* BuildTIN::locate_special(const float* p, TINtriangle* t)
{
	do
	{
		assert(t->V[0] == 0);
...
#define MAX_LOCATE_STEPS 10000
 ...
 TINtriangle* BuildTIN::TINlocate(float* p)
{
	int steps = 0;
	double d;
	int c1;
...
#define stkDESTROY() { if (dfs_stack) free(dfs_stack); dfs_stack = 0; dfs_stack_cur = 0; dfs_stack_max = 0; }
 ...
 void BuildTIN::TINupdate(float* p, TINtriangle* t)
{
 ...
 bool BuildTIN::TINinsert(float* p)
{
...
int BuildTIN::TINget_size()
{
	return triangle_buffer_size;
}

TINtriangle* BuildTIN::TINget_triangle(int t)
{
	return triangle_buffer + t;
}

void BuildTIN::TINclean(int num, bool convex_hull)
{
	...
	if (triangle_buffer_alloc < num * 2)
	{
		...
		{
			fprintf(stderr, "ERROR (TINclean): failed malloc for %d TINtriangles\n", triangle_buffer_alloc);
			exit(1);
		}
	}
...
void BuildTIN::initialize_endianness()
{
	int i = 1;
...
void BuildTIN::to_big_endian(int* value)
{
	if (is_little_endian)
	{
		char help;
...
void BuildTIN::to_little_endian(int* value)
{
	if (!is_little_endian)
	{
		char help;
...
void BuildTIN::to_big_endian(double* value)
{
	if (is_little_endian)
	{
		char help;
...
void BuildTIN::to_little_endian(double* value)
{
	if (!is_little_endian)
	{
		char help;
...
void BuildTIN::create_dbf_file(const char* file_name)
{
	FILE* file_dbf = fopen(file_name, "wb");
	if (file_dbf)
	{
		unsigned char	dbf_header[32];
		for (int i = 0; i < 32; i++) dbf_header[i] = 0;
		/* version */
		dbf_header[0] = 0x03;
		/* dummy date */
		dbf_header[1] = 0x09; /* YY */
		dbf_header[2] = 0x09; /* MM */
		dbf_header[3] = 0x09; /* DD */
		/* number of records in the table (least significant byte first) */
		dbf_header[4] = 0x00;
		dbf_header[5] = 0x00;
		dbf_header[6] = 0x00;
		dbf_header[7] = 0x00;
		/* number of bytes in header (least significant byte first) */
		dbf_header[8] = (unsigned char)(33 % 256);
		dbf_header[9] = (unsigned char)(33 / 256);
		/* number of bytes in the record (least significant byte first) */
		dbf_header[10] = (unsigned char)(1 % 256);
		dbf_header[11] = (unsigned char)(1 / 256);
		/* language driver ID */
		dbf_header[29] = 87;
		/* write the 32 byte file header */
		fwrite(dbf_header, 32, 1, file_dbf);
		/* write the newline character */
		char newline = 0x0d;
		fwrite(&newline, 1, 1, file_dbf);
		/* done */
		fclose(file_dbf);
	}
}
